...
3. **组内 OR、组间 AND**：用户在每个大类标签里勾选若干原子条件；**同一大类内勾选的条件取 OR**（任一满足即该类放行），**6 大类之间取 AND**（全部放行才出买点）。
4. **无勾选 = 该类恒真**（放行）。所以默认全勾 = 几乎不筛；要精确选股得在各组里主动勾选并理解 OR 语义。
...
**关键坑（用户常误解）**：以为"勾越多越严"——错。组内是 **OR**，勾多个 = 该类更容易放行；要严得**每组只勾 1 条最硬的条件**，或理解成"满足任一组内任一条件即过该类"。

---

## 置信度
- **铁证**：73 个原子条件公式全部来自 EXE `.rdata` 明文常量（地址见上）；6 组 `:=1` 硬编码；复合 AND 门 `0x00894bc8`。
- **高置信**：组内 OR / 组间 AND 语义（由复合公式结构 + UI 勾选对话框 + `FUN_008a5b30` 条件工厂查表推出）。
